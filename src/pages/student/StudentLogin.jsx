...
function Login() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            let session;
            try {
                session = await authService.login(formData)
            } catch (err) {
                const msg = err?.message || ''
                // If a session is already active, try to clear it and retry once
                if (msg.includes('Creation of a session is prohibited') || msg.toLowerCase().includes('session is active')) {
                    console.warn('Active session detected during login, clearing sessions and retrying...')
                    try { await authService.logout() } catch (e) { if (process.env.NODE_ENV === 'development') console.debug('Logout during retry failed', e) }
                    session = await authService.login(formData)
                } else throw err
            }

            console.debug('Login session result:', session)

            if (session) {
                // B. Get User Identity (retry once if session hasn't propagated yet)
                ...
                if (!userData) {
                    setError("Login failed: unable to verify session. Please try again.")
                    setLoading(false)
                    return
                }
                ...
                console.debug('User role fetch result:', userDocs)
                let role = 'student'
            ...
            // Give a more specific message where possible
